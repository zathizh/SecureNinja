...
void thread_function(int id) { // id is 0 or 1
	// We transfer money from our account to the other account. int amount = (id + 2) * 10;
	int lo_id = id;
	int hi_id = !id;
	if (lo_id > hi_id) {
		int tmp = lo_id;
	...
	lock_guard<mutex> this_lock(locks[lo_id]);
	lock_guard<mutex> that_lock(locks[hi_id]);
	accounts[id] -= amount;
	accounts[!id] += amount;
